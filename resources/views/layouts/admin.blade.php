...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .admin-main { flex: 1; min-width: 0; padding: 34px clamp(22px, 4vw, 58px); }
        ...
        .language-switcher { display: inline-flex; align-items: center; gap: 8px; border: 1px solid var(--line); border-radius: 999px; padding: 4px; color: var(--brand-dark); background: #fff; box-shadow: 0 5px 14px rgba(55,111,107,.08); cursor: pointer; font-weight: 700; transition: .2s ease; }
        .language-switcher:hover { border-color: var(--brand); box-shadow: 0 7px 18px rgba(55,111,107,.14); }
        .language-option { min-width: 72px; padding: 8px 12px; border-radius: 999px; text-align: center; }
        .language-option.active { color: #fff; background: var(--brand-dark); }
...
<body>
    ...
    <script>
        ...
        function setDashboardLanguage(language) {
            ...
            document.querySelectorAll('[data-language-option]').forEach((element) => element.classList.toggle('active', element.dataset.languageOption === language));
